...
class SignUp extends Component {
  ...
  idBlur = event => {
    if (this.state.id) {
      axios.get(`/api/user/idCheck?id=${event.target.value}`).then(res => {
  ...
  passwordBlur = event => {
    if (this.state.password) {
      ...
      if (rule.test(this.state.password)) {
        ...
        this.setState({
          passwordInfo: "8~16자 영문, 숫자, 특수문자를 사용하세요."
        });
      }
    } else {
  ...
  passwordCheckBlur = event => {
    if (this.state.passwordCheck) {
      if (this.state.password === this.state.passwordCheck)
        this.setState({
          passwordCheckInfo: "비밀번호가 일치합니다.",
          passwordCheckPossible: true
        });
      else
        this.setState({
          passwordCheckInfo: "비밀번호가 일치하지 않습니다.",
          passwordCheckPossible: false
        });
    } else
      this.setState({
        passwordCheckInfo: "필수 정보입니다.",
        passwordCheckPossible: false
      });
  };
  ...
  mailBlur = event => {
    if (this.state.mail) {
      ...
      if (rule.test(this.state.mail)) {
        axios
          .get(`/api/user/mailCheck?mail=${event.target.value}`)
          .then(res => {
  ...
  phoneBlur = event => {
    if (this.state.phone) {
      ...
      if (rule.test(this.state.phone)) {
        axios
          .get(`/api/user/phoneCheck?phone=${event.target.value}`)
          .then(res => {
            this.setState({ phoneInfo: res.data.info });
          });
      } else
        this.setState({
          phoneInfo: "형식이 올바르지 않은 휴대전화 번호입니다."
        });
