...
const getHtml = async (theatercode, date) => {
  try {
    return await axios.get(
      `http://www.cgv.co.kr/common/showtimes/iframeTheater.aspx?theatercode=${theatercode}&date=${date}`
    );
  } catch (e) {
    console.error(e);
  }
    ...
    getHtml(theater.theatercode, date)
        ...
        $bodyList.each((i, elem) => {
          res[i] = {
            name: $(elem)
              ...
              .trim(),
            grade: $(elem)
              .find("div.info-movie > span.ico-grade")
              .text()
              .trim(),
            genre: $(elem)
              .find("div.info-movie > i")
              .eq(0)
              .text()
              .trim(),
            runningtime: $(elem)
          ...
          $screenList.each((j, elem) => {
            ...
            const $timetableList = $(elem)
              .find("div.info-timetable > ul")
              .children("li");
      ...
      .then(res => {
        mkdirp(`../시간표/${city.areaname}/${theater.theatername}/`)
          .then(() => {
            fs.writeFileSync(
              `../시간표/${city.areaname}/${theater.theatername}/${date}.json`,
              JSON.stringify(res),
              "utf8"
            );
