...
router.post("/", isLoggedIn, async (req, res) => {
  ...
  const num = await select(
    "select count(*) num from reservation join timetable on timetable.id = reservation.timetable_id join screen on timetable.screen_id = screen.id join theater on theater.id = screen.theater_id where theater.id = ?",
    [theater_id]
  );
  ...
  let price = adult * 10000 + teenager * 7000;
  let querys = [];
  querys.push({
    sql:
      "insert into reservation(id, user_id, timetable_id, payment_type, price) values(?,?,?,?,?)",
...
router.get("/reserinfo", async (req, res) => {
  const tickets = await select(
    "select reservation.id reservationcode, reservation.payment_type, reservation.price, movie.id movieid, movie.movie_title movietitle, movie.running_time runningtime, timetable.screen_type screentype, timetable.start_date startdate, timetable.start_time starttime, theater.id theaterid, theater.name theatername, screen.id screenid, screen.name screenname, ticket.seat_id seatid from reservation join timetable on timetable.id = reservation.timetable_id join movie on movie.id = timetable.movie_id join screen on screen.id = timetable.screen_id join theater on theater.id = screen.theater_id join ticket on ticket.reservation_id = reservation.id where reservation.user_id = ?",
    ["kwon4450"]
  );
  const data = [];
