...
router.get('/', async (req, res) => {
  let data = [];
  const index ={"01": 0, "02":1, "202": 2, "12": 3, "03,205": 4, "11": 5, "05,207": 6, "204": 7, "206,04,06": 8};
  ...
  const theaters = await select("select * from theater inner join area on theater.areacode = area.areacode");
...
router.get('/timetable', async (req, res) => {
  ...
  const timetables = await select("select movie.id movieid, movie.movie_title movietitle, movie.grade grade, movie.genre genre, movie.runnung_time runningtime, movie.opening_date releasedate, screen.id screenid, screen.name screenname, timetable.id timetableid, timetable.screen_type screentype, screen.totalseats totalseats, timetable.start_time starttime from theater join screen on theater.id = screen.theater_id join timetable on screen.id = timetable.screen_id join movie on movie.id = timetable.movie_id where theater.id = ? and timetable.start_date = ? order by starttime", [theatercode, date]);
  const data = [];
  for (const timetable of timetables) {
    ...
    let endtime = String(parseInt(timetable.starttime) + timetable.runningtime + 10);
    while(endtime.length != 4) endtime = "0" + endtime;
    const timetableindex = data[movieindex].screenList[screenindex].timetableList.length;
    ...
    let seatList = []
    const seats = await select("select ticket.id ticketid, seat.id seatid, seat.row_index, seat.col_index, seat.row_num, seat.col_num from seat left outer join ticket on ticket.seat_id = seat.id inner join screen on screen.id = seat.screen_id inner join timetable on timetable.screen_id = screen.id where timetable.id = ?", [timetable.timetableid]);
    for (const seat of seats) {
      let index = -1;
      for (let i = 0; i < seatList.length; i++) {
        if (seatList[i].row_i == seat.row_index && seatList[i].col_i == seat.col_index) {
          index = i;
          break;
        } 
      }
    ...
    data[movieindex].screenList[screenindex].timetableList[timetableindex].seatList = seatList;
  }
  return res.json(data);
})
